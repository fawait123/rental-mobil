...
@section('content')
    <div class="row">
        <div class="col-lg-12 mb-30">
            <div class="card">
                <div class="card-body">
                    <form action="{{ isset($id) ? route('recomended.update', $recomended->id) : route('recomended.store') }}"
                        method="post">
                        @csrf
                        @if (isset($id))
                            @method('put')
                            ...
                            <select name="property_id" id="property_id"
                                ...
                                @foreach ($property as $item)
                                    <option value="{{ $item->id }}"
                                        {{ (isset($id) ? ($recomended->property_id == $item->id ? 'selected' : '') : old('property_id') == $item->id) ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-25">
                            ...
                            <select name="is_show" id="is_show"
                                ...
                                <option value="">pilih</option>
                                <option value="1"
                                    {{ (isset($id) ? ($recomended->is_show == '1' ? 'selected' : '') : old('is_show') == '1') ? 'selected' : '' }}>
                                    Yes</option>
                                <option value="0"
                                    {{ (isset($id) ? ($recomended->is_show == '0' ? 'selected' : '') : old('is_show') == '0') ? 'selected' : '' }}>
                                    No</option>
                            </select>
                            @error('is_show')
                                <div class="invalid-feedback">{{ $message }}</div>
